...
public class VentanaTablero extends JFrame {
		...
		if (pnTablero == null) {
			...
			for (int i = 0; i < 9; i++) {
				for (int j = 0; j < 9; j++) {
					...
					tablero[i][j].action(v, i, j, jugada);
	...
	private void deshabilitarBotones() {
		...
		tablero[0][4].addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				VentanaVolverATirar v = new VentanaVolverATirar(jugada, ventana);
				v.setVisible(true);
			}
		});
	...
	private void colocarJugadores() {
		...
		System.out.println(jugada.getJugadores().size());
	}
